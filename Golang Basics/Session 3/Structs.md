...
func main() {
	...
	p4.firstName = "Folan" // (*p4).firstName = "Folan". Go dereferences the p4 by itself and we don't have to so it.

	/* A nil pointer to a Person struct.
	You would need to assign it a valid reference (either through new(Person) or by assigning it the address of an existing Person).
	*/
	var p5 *Person
}
```