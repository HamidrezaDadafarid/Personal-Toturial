...
func main() {
	...
	// you can still append to a nil slice
	...
	fmt.Println(v3[9])
	fmt.Println(v3[10]) // slices range is from 0 to (length - 1). panic!

	/*
	when we append one or more elemtents to slice, if there is capacity for them it will be added.
	if not, a copy of that array with the length 2 times the current length will be made.
	*/
